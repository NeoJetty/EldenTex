...
function requestUntaggedTextureData(userID: number, tagID: number): Promise<any> {
    if(AppConfig.debug.level > 0) console.log(`Server request: /untaggedTexture/${userID}/${tagID}`);
...
function requestTextureData(textureID: number): Promise<any> {
    if(AppConfig.debug.level > 0) console.log(`Server request: /textureData/${textureID}`);
...
function requestMultiFilterTextureData(filterData: { tags: { tag_id: number, vote: boolean }[] }): Promise<any[]> {
    ...
    if(AppConfig.debug.level > 0) console.log(`Server request: /serveTexturesByMultipleTags POST: ${dataWithUserID}`);
