...
class TextureDataContainer {
  ...
  public updateFromImageDataJSON(data: any): void {
    ...
    this.jpgURL = AppConfig.getInstance().buildJPGPath(
      data.textureName,
      this.textureTypes
    );
...
class AppConfig {
  ...
  public buildJPGPath2(textureName: string, textureTypes: string[]): string {
    let basePath = this.folders.jpgs + textureName;

    if (textureTypes.includes("_n")) {
      return basePath + "_n.jpg";
    }
    ...
    if (textureTypes.length === 0) {
      ...
      return basePath + textureTypes[0] + ".jpg";
    }
  }

  public buildLowQualityJPGPath(
    textureName: string,
  ...
  // Method to update from JSON
  public updateFromImageDataJSON(
    data: any,
    propertyGroup: TextureDataContainer
  ): void {
    ...
    propertyGroup.jpgURL = this.buildJPGPath(
      data.textureName,
      propertyGroup.textureTypes
    );
