 ...
 * @returns {void}
 */
async function populateAllNeutralTags(tagContainer: HTMLDivElement, preCheckedTagIDs: TagVote[] = []) {
    try {
        // Fetch all tags from the server
 ...
 */
function on3StateToggleClickDBUpdate(event: MouseEvent, textureID: number): void {
 ...
 * @param {MouseEvent} event - The click event.
 */
function on3StateToggleClickHTMLChangeOnly(event: MouseEvent): void {
    const toggle = event.currentTarget as HTMLElement; // Get the toggle element from the event
    const toggleImage = toggle.querySelector('.toggle-image') as HTMLImageElement;
...
function handleTagSelection(isChecked: boolean, tagID: number, textureID: number): void {
    ...
    const url = `/api/dbAddTagToTexture?user_id=${userID}&tag_id=${tagID}&texture_id=${textureID}&vote=${action}`;
...
function handleTagNeutralSelection(tagID: number, textureID: number): void {
    ...
    const url = `/api/dbDeleteTagFromTexture/${userID}/${tagID}/${textureID}`;
...
async function requestTagsForImage(textureID: number): Promise<TagVote[]> {
    ...
    try {
        const response = await fetch(`/api/serveTagsForTexture/${userId}/${textureID}`);
        if (!response.ok) {
