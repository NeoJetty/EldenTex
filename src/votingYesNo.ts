...
// Set the variables
let voteTagTheme: string = 'Symbol';
let voteTagID: number = 4;

// Function to create the voting UI
function createVotingUI(parentDiv: HTMLElement): void {
...
function handleVote(isYes: boolean): void {
    ...
    const url = `/api/dbAddTagToTexture?user_id=${userID}&tag_id=${voteTagID}&texture_id=${imageID}&vote=${voteValue}`;
...
function cycleTextureTypeTabs(): void {
    const activeTabs = document.querySelectorAll('.tex-type-navitem.highlighted'); // Only consider highlighted tabs
    const activeTab = Array.from(activeTabs).find(tab => tab.classList.contains('active')) as HTMLElement | undefined;

    if (activeTab) {
        // Find the next tab or loop back to the first one
        const nextTab = activeTab.nextElementSibling as HTMLElement || activeTabs[0] as HTMLElement;
...
function nextVote(): void {
    console.log('Next vote triggered');
    // Simulate a click on the first tab to reload its content
    const firstTabLink = document.querySelector('.tab-link[data-tab="tab1"]') as HTMLElement | null;
    if (firstTabLink) {
        firstTabLink.click();
    }
}
...
function removeKeyPressListener(): void {
