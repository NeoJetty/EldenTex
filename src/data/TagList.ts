...
import {
  ...
  submitFilterSearch,
} from "./api/requestTagRelatedData.js";
...
class TagList {
  ...
  constructor(
    ...
    callback: (filter: TagVote[]) => void
  ) {
    this.tagContainer = tagContainer;
    this.textureUpdateCallback = callback;
  }

  public async buildFilterSelection(
    preCheckedTagIDs: TagVote[] = []
  ): Promise<void> {
    try {
      //--------------------------------------
      //           Dropdown
  ...
  private createToggleForTag(
    ...
    const initialState = tagVote
      ? translateToToggleState(tagVote)
      : ToggleState.NEUTRAL;
  ...
  private async createFavouriteFiltersDropdown(): Promise<void> {
    try {
      ...
      selectElement.addEventListener("change", () =>
        this.onDropDownChange(selectElement)
      );
    } catch (error) {
  ...
  private async onDropDownChange(selectElement: HTMLSelectElement) {
    ...
    const selectedSearch = this.savedSearches.find(
      (search) => search.search_name === selectedFilter
    );
    if (!selectedSearch) {
